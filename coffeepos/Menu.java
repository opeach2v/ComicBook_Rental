...
public class Menu {
    // 메뉴 번호, 메뉴 이름
    public static HashMap<Integer, String> menuNum = new HashMap<>();

    // 메뉴 이름, {가격, 수량}
    public static HashMap<String, ArrayList<Integer>> drinkMenus = new HashMap<>();
    public static HashMap<String, ArrayList<Integer>> desertMenus = new HashMap<>();
    ...
    public Menu() { // 메뉴 초기화 해주는 용도
        ...
        for(int i = 1; i <= drinks.length; i++) {
            menuNum.put(i, drinks[i - 1]);  // 0번은 없음
        }
        for(int i = idx; i < deserts.length; i++) {
            menuNum.put(i, deserts[i - 1]); // 이어서 디저트 넣기
        }
        ...
        for(int i = 0; i < drinks.length; i++) {
            ...
            drinkMenus.put(drinks[i], drinksList);
        }
        ...
        for(int i = 0; i < deserts.length; i++) {
            ...
            desertMenus.put(deserts[i], desertList);
        }
    }
    ...
    public static int printMenu() {
        ...
        int idx = 1;
        for(String name : drinkMenus.keySet()) {
            System.out.print("  " +idx +". " +name +" - " +drinkMenus.get(name).getFirst());  // 이름, 가격 출력
            if(drinkMenus.get(name).getLast() == 0) {   // 수량이 0이면
        ...
        System.out.println("\n        < 디저트 >");
        for(String name : desertMenus.keySet()) {
            System.out.print("  " +idx +". " +name +" - " +desertMenus.get(name).getFirst());  // 이름, 가격 출력
            if(desertMenus.get(name).getLast() == 0) {   // 수량이 0이면
                System.out.print("  (품절)");     // 품절 띄우기
            }
            System.out.println();
            idx++;
    ...
    public static int choiceMenu() {
        ...
        while(choice != 'e' && choice != 'p') {
            ...
            if((choice >= 1 && choice <= 9) && drinkMenus.get(menuNum.get(choice)).getLast() > 0) {   // 수량이 0이 아니면
